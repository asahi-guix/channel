...
(define-public asahi-scripts
  (package
             ...
             (let ((out (assoc-ref outputs "out")))
               ...
               (substitute* "update-m1n1"
                 (("/usr/share/asahi-scripts/functions.sh")
                  (string-append out "/share/asahi-scripts/functions.sh"))
                 (("/usr/lib/asahi-boot/")
                  (string-append (assoc-ref inputs "asahi-m1n1") "/usr/lib/asahi-boot/"))
                 (("\\$SOURCE/u-boot-nodtb.bin")
                  (string-append (assoc-ref inputs "u-boot-apple-m1") "/libexec/u-boot-nodtb.bin"))
                 (("/lib/modules/\\*-ARCH/dtbs/\\*.dtb")
                  (string-append (assoc-ref inputs "u-boot-apple-m1") "/libexec/arch/arm/dts/*.dtb"))))))
         (replace 'configure
           (lambda* (#:key inputs outputs #:allow-other-keys)
    ...
    (inputs (list asahi-fwextract asahi-m1n1 lzfse python u-boot-apple-m1))
